...
        description, amount, category, paidBy, participants: finalParticipants, group: group || null
    });
...
            { $group: { 
                _id: null, 
                toPay: { $sum: { $cond: [{$lt: ["$amount", 0]}, "$amount", 0]}}, 
                toReceive: { $sum: { $cond: [{$gt: ["$amount", 0]}, "$amount", 0]}}
            }}
        ]);
        
        const balances = {
            toPay: Math.abs(balancesData[0]?.toPay || 0),
            toReceive: balancesData[0]?.toReceive || 0
        };
